...
2. **Branding materials** — logo (PNG, ≤150×150px) and primary/secondary colors in hex.
3. **Sender-domain DNS (email auth)** — to send payer emails from the merchant's own domain (e.g. `ar@merchant.com`), the merchant adds the DKIM/SPF DNS records Paystand provides, then Paystand verifies the domain. Until the domain is verified, emails send from `noreply@paystand.com` / `support@paystand.com` and are more likely to hit spam.
...
- **The defaulting must have a source.** The value must come from somewhere in setup for the automated posting to know when to apply it — otherwise it has to be part of the import. Two working patterns: (a) default the dimension **on the GL account** so the posting inherits it from the referenced account, or (b) include the value in the import data.
- Apply the default in **both the TEST/sandbox and PROD environments** — not just one — and validate by reversing and reposting a test journal entry.
