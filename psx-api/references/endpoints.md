...
| POST | `/payments/secure` | Submit a payment against a verified bank (`bankId`) OR saved card (`cardId`), plus `payerId`, `amount`, `currency`. Optional `receivableId` applies it to an invoice. To apply the convenience fee, also pass `feeSplit: {subtotal, feeSplitType}` from a prior `splitFees` call, taken from the rail that matches the payment method (`cardPayments` for a card, `networkBankPayments` for a verified bank; see gotchas.md) — without it, no convenience fee is applied. |
...
| Apply the convenience fee on an API charge | Not automatic (only at hosted checkout) | Call `POST /feeSplits/splitFees`, then charge the `payerTotal` of the rail that matches the payment method via `POST /payments/secure` |
